...
function addNewRecord2() {
    ...
    const cells = [
        `<textarea name="data_registro" class="form-control textarea-fixed-size col-md-6"></textarea>`,
        `<textarea name="contato" class="form-control textarea-fixed-size col-md-6"></textarea>`,
...
function saveTableData2() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const rowData = {
            data_registro: row.cells[0].children[0].value || '',
            contato: row.cells[1].children[0].value || ''
        };
        data.push(rowData);
    }

    document.getElementById('linhaDinamicaDataset2').value = JSON.stringify(data);
}

// Função para carregar os dados da Tabela 2
function loadTableData2() {
    const data = JSON.parse(document.getElementById('linhaDinamicaDataset2').value || '[]');
    const tabela = document.getElementById('tabelaRegistross').getElementsByTagName('tbody')[0];
    ...
    data.forEach((item) => {
        ...
        newRow.insertCell(0).innerHTML = `<textarea name="data_registro" class="form-control textarea-fixed-size col-md-6">${item.data_registro || ''}</textarea>`;
        newRow.insertCell(1).innerHTML = `<textarea name="contato" class="form-control textarea-fixed-size col-md-6">${item.contato || ''}</textarea>`;
        newRow.insertCell(2).innerHTML = `<button type="button" onclick="removeRow2(this)">Remover</button>`;
    });
}

// Função para salvar todas as tabelas (incluindo Tabela 2)
function saveAllTables2() {
