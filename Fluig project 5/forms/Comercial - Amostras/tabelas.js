...
function addNewRecord() {
    ...
    const cells = [
        `<textarea id="codigo_${index}" name="codigo_${index}" class="form-control col-md-3"></textarea>`, // Alterado para textarea
...
function saveTableData() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const rowData = {
            codigo: row.cells[0].children[0].value || '', // Pega o valor do textarea
...
function loadTableData() {
    ...
    data.forEach((item, index) => {
        ...
        newRow.insertCell(0).innerHTML = `<textarea id="codigo_${index}" name="codigo_${index}" class="form-control col-md-3">${item.codigo || ''}</textarea>`; // textarea carregando dados
